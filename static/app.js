...
function toggleSelection(ids){
  ...
  for(const id of refs){if(selected.has(id))selected.delete(id);else{selected.add(id);activeEntityId=id;}}
  normalizeActiveSelection();inspect();updateButtons();
}
function selectionBoxElement(){
  let element=document.querySelector('#selectionBox');
  if(element)return element;
  element=document.createElement('div');element.id='selectionBox';
  Object.assign(element.style,{position:'fixed',display:'none',pointerEvents:'none',zIndex:'9999',border:'1px solid rgba(120,190,255,.9)',background:'rgba(80,150,220,.12)'});
  document.body.appendChild(element);return element;
}
function updateSelectionBoxOverlay(box){const element=selectionBoxElement(),left=Math.min(box.startX,box.currentX),top=Math.min(box.startY,box.currentY),width=Math.abs(box.currentX-box.startX),height=Math.abs(box.currentY-box.startY);element.style.display='block';element.style.left=`${left}px`;element.style.top=`${top}px`;element.style.width=`${width}px`;element.style.height=`${height}px`;}
function clearSelectionBoxOverlay(){const element=document.querySelector('#selectionBox');if(element)element.style.display='none';}
function entitiesInSelectionBox(box){
  const density=devicePixelRatio||1,left=Math.min(box.startX,box.currentX)*density,right=Math.max(box.startX,box.currentX)*density,top=Math.min(box.startY,box.currentY)*density,bottom=Math.max(box.startY,box.currentY)*density,vp=viewProjection(),visible=visibleEntityIds();
  return assertWorkspace().entities.filter(entity=>{if(!visible.has(entity.id))return false;const center=project(entity.position,vp);return center&&center[0]>=left&&center[0]<=right&&center[1]>=top&&center[1]<=bottom;});
}
function inspect(){
  const entities=assertWorkspace().entities.filter(entity=>selected.has(entity.id));
...
canvas.ondblclick=event=>{if(ws===null)return;if(event.button!==0)return;const entity=pickEntity(event.clientX,event.clientY);if(!entity)return;lookAtEntityId=entity.id;assertWorkspace().camera.reference=[...entity.position];syncCameraAnglesToActive();status(`lookAt: ${entity.id}`);};
canvas.onwheel=event=>{if(ws===null)return;event.preventDefault();const step=Math.sign(event.deltaY)*requiredNumber(cameraSettings(),'wheel_zoom_speed','settings.camera_defaults'),target=cameraReference(),offset=V.sub(assertWorkspace().camera.position,target),distance=V.length(offset);if(distance<=1e-6)throw new Error('camera cannot zoom from its reference point');assertWorkspace().camera.position=V.add(target,V.mul(V.norm(offset),Math.max(.25,distance+step)));syncCameraAnglesToActive();};
canvas.onmousedown=event=>{if(ws===null)return;if(event.button===2){beginOrbit(event.clientX,event.clientY);return;}if(event.button!==0)return;const axis=gizmoAxisHit(event.clientX,event.clientY);if(axis&&selected.size){if(lookAtEntityId&&selected.has(lookAtEntityId))detachLookAtReference();beginAxisDrag(axis,event.clientX,event.clientY);return;}const entity=pickEntity(event.clientX,event.clientY);if(linkSource){if(entity&&entity.id!==linkSource){linkTarget=entity.id;openLinkTypePopup();return;}if(!entity){pan={x:event.clientX,y:event.clientY,moved:false};last=[event.clientX,event.clientY];}return;}if(!entity){if(event.ctrlKey){event.preventDefault();boxSelection={startX:event.clientX,startY:event.clientY,currentX:event.clientX,currentY:event.clientY};updateSelectionBoxOverlay(boxSelection);return;}pan={x:event.clientX,y:event.clientY,moved:false};last=[event.clientX,event.clientY];return;}if(event.ctrlKey||event.shiftKey){toggleSelection(entity.id);}else{selected=new Set([entity.id]);setActiveEntity(entity.id);inspect();updateButtons();}};
window.onmouseup=event=>{orbit=null;dragAxis=null;if(boxSelection){const box=boxSelection;boxSelection=null;clearSelectionBoxOverlay();if(event.button===0){const ids=entitiesInSelectionBox(box).map(entity=>entity.id);if(ids.length)toggleSelection(ids);status(`box toggled ${ids.length} nodes`);}return;}if(pan){const click=!pan.moved;pan=null;if(click&&event.button===0&&!event.ctrlKey&&!event.shiftKey&&!linkSource){selected.clear();activeEntityId=null;inspect();updateButtons();}}};
window.onmousemove=event=>{if(ws===null)return;if(orbit){updateOrbit(event.clientX,event.clientY);hovered=null;return;}if(boxSelection){boxSelection.currentX=event.clientX;boxSelection.currentY=event.clientY;updateSelectionBoxOverlay(boxSelection);hovered=null;return;}if(pan){const dx=event.clientX-last[0],dy=event.clientY-last[1],total=Math.abs(event.clientX-pan.x)+Math.abs(event.clientY-pan.y);if(!pan.moved&&total>2){pan.moved=true;detachLookAtReference();}if(pan.moved){const speed=requiredNumber(cameraSettings(),'drag_pan_speed','settings.camera_defaults'),delta=V.add(V.mul(cameraRight(),-dx*speed),V.mul(cameraUp(),dy*speed));assertWorkspace().camera.reference=V.add(cameraReference(),delta);assertWorkspace().camera.position=V.add(assertWorkspace().camera.position,delta);syncCameraAnglesToActive();}last=[event.clientX,event.clientY];hovered=null;return;}if(dragAxis&&selected.size){const dx=event.clientX-last[0],dy=event.clientY-last[1],screenDelta=dx*dragAxis.screenDirection[0]+dy*dragAxis.screenDirection[1];dragAxis.total+=screenDelta*dragAxis.worldPerPixel;const spacing=gridSize();for(const entity of assertWorkspace().entities){const start=dragAxis.startPositions.get(entity.id);if(!start)continue;const raw=start[dragAxis.axisIndex]+dragAxis.total;entity.position[dragAxis.axisIndex]=viewSettings().snap_to_grid?Math.round(raw/spacing)*spacing:raw;}last=[event.clientX,event.clientY];inspect();return;}hovered=pickEntity(event.clientX,event.clientY)?.id??null;};

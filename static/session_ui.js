...
(function installStructureSessionUI(){
  ...
  newInstance=function(){
    ...
    base.master_ref=masters()[0]?.id||'master-1';
    base.semantic_projection_style='topic';
    base.visual_style=base.projection_style||visualStyles()[0]?.id||'atlas';
  ...
  instancePayload=function(){return S.instances.map(raw=>{
    ...
    return {id:i.id,name:i.name,master_ref:i.master_ref,semantic_projection_style:i.semantic_projection_style,scope_type:i.scope_type,scope_ref:i.scope_ref,visual_style:i.visual_style,projection_dimension:i.projection_dimension,relation_depth:i.relation_depth,impact_depth:i.impact_depth||32};
  })};

  const oldInstanceHTML=instanceHTML;
  instanceHTML=function(raw){
    const inst=normalize(raw), html=oldInstanceHTML(inst), parser=document.createElement('template');parser.innerHTML=html.trim();const card=parser.content.firstElementChild;
    const body=card.querySelector('.instance-body');
    const oldGrid=body.querySelector('.grid3')||body.querySelector('.grid2');
    const oldDepth=body.querySelector('[data-key="dependency_depth"]')?.closest('.field');
    if(oldGrid)oldGrid.remove(); if(oldDepth)oldDepth.remove();
    ...
    const nameField=body.querySelector('.field');nameField?.insertAdjacentElement('afterend',controls);
  ...
  renderInstances=function(){
    ...
    $('#instances').querySelectorAll('[data-session]').forEach(el=>{
      el.onchange=()=>{
        ...
        if(key==='visual_style'){inst.projection_style=inst.visual_style;const dims=visualFor(inst)?.dimensions||['2d'];if(!dims.includes(inst.projection_dimension))inst.projection_dimension=dims.includes('3d')?'3d':dims[0];}
        normalize(inst);renderInstances();scheduleSceneReload();
      };
    });
  };

  async function refreshSessionCatalog(){
    if(!S.sourceSpec)return;
    const q=new URLSearchParams();
    if(S.sourceSpec.type==='directory'){q.set('source_type','directory');q.set('source_path',S.sourceSpec.path||'');}
    else{q.set('source_type','github');q.set('repo',S.sourceSpec.repo||'Th3Hypn0tist/AIGMos_docs');q.set('branch',S.sourceSpec.branch||'main');}
    S.catalog=await getJSON(`/api/projection-catalog?${q}`);
    S.instances.forEach(normalize);renderInstances();
  }

  const oldLoadScene=loadScene;
  loadScene=async function(){
    if(!S.instances.length)return oldLoadScene();
    setStatus('loading');$('#error').textContent='None.';
    try{
      if(!S.catalog?.projection_styles)await refreshSessionCatalog();
      const source=S.sourceSpec||{type:'github',repo:'Th3Hypn0tist/AIGMos_docs',branch:$('#branch')?.value||'main'};
      ...
      S.scene=data.scene;S.catalog=data.catalog||S.catalog;S.source=data.masters?.[0]?.source||{};
      ...
      setStatus(`${S.scene?.composition?.projection_count||S.instances.length} projections · ${S.scene?.objects?.reduce((n,o)=>n+(o.nodes?.length||0),0)||0} nodes`);
    }catch(e){showError(e)}
  };

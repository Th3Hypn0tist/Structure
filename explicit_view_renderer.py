...
def build_explicit_view_geometry(view: dict[str, Any]) -> dict[str, Any]:
    ...
    for section in view.get("sections", []):
        ...
        for block in section.get("blocks", []):
            ...
            if template == "chip_grid":
                cols, item_gap, item_h = 8, 8, 34
                item_w = int((inner_w - (cols - 1) * item_gap) / cols)
                rows = max(1, (len(items) + cols - 1) // cols)
                content_h = rows * item_h + max(0, rows - 1) * item_gap
            elif template == "registry_grid":
                cols, item_gap, item_h = 3, 14, 82
                item_w = int((inner_w - (cols - 1) * item_gap) / cols)
                rows = max(1, (len(items) + cols - 1) // cols)
                content_h = rows * item_h + max(0, rows - 1) * item_gap
            else:
                cols, item_gap = (3 if len(items) <= 9 else 4), 14
                item_w = int((inner_w - (cols - 1) * item_gap) / cols)
                heights = [_item_height(template, item) for item in items] or [92]
                rows_list = [heights[i:i + cols] for i in range(0, len(heights), cols)]
                content_h = sum(max(row) for row in rows_list) + max(0, len(rows_list) - 1) * item_gap

            block_h = block_title_h + content_h + 22
            block_occurrences.append({
                "kind": "block", "id": block_id, "parent_id": section_id,
                "title": block.get("title", block["id"]), "template": template,
                "accent": block.get("accent", section.get("accent", "primary")),
                "status": block.get("status"), "x": margin + section_pad, "y": block_y,
                "width": inner_w, "height": block_h, "child_count": len(items),
            })
            ...
            if template in ("chip_grid", "registry_grid"):
                for index, item in enumerate(items):
                    ...
                    actual_h = 34 if template == "chip_grid" else 82
                    item_x = margin + section_pad + col * (item_w + item_gap)
                    item_y = item_y0 + row * (actual_h + item_gap)
                    block_occurrences.append({
                        "kind": "item", "role": item.get("role", "card"),
                        "id": f"{block_id}::{index}", "parent_id": block_id,
                        "title": item.get("title", ""), "summary": item.get("summary", ""),
                        "status": item.get("status"), "value": item.get("value"),
                        "provenance": item.get("provenance"), "x": item_x, "y": item_y,
                        "width": item_w, "height": actual_h,
                ...
                for row_start in range(0, len(items), cols):
                    ...
                    for col, item in enumerate(row_items):
                        ...
                        block_occurrences.append({
                            ...
                            "title": item.get("title", ""), "summary": item.get("summary", ""),
                            "status": item.get("status"), "value": item.get("value"),
                            "provenance": item.get("provenance"), "x": item_x, "y": row_y,
                            "width": item_w, "height": _item_height(template, item),
                            "child_count": int(item.get("child_count", 0) or 0),
                        })
        ...
        occurrences.append({
            "kind": "section", "id": section_id, "parent_id": None,
            "title": section.get("title", section_id), "subtitle": section.get("subtitle", ""),
            "template": section.get("template", "large_frame"), "accent": section.get("accent", "primary"),
            "status": section.get("status"), "x": margin, "y": section_y,
            "width": usable_w, "height": section_h, "child_count": len(section.get("blocks", [])),
        })
        occurrences.extend(block_occurrences)
        y += section_h + section_gap

    return {
        "kind": "explicit_view_geometry", "view_id": view.get("id"),
        "title": view.get("title"), "subtitle": view.get("subtitle"), "status": view.get("status"),
        "view_box": {"width": width, "height": max(900, y + 60)}, "occurrences": occurrences,
        "navigation": {

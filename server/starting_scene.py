...
def _function(
    ...
    **properties: Any,
    ...
    value: dict[str, Any] = {
        "function_type_ref": function_type_ref,
        "properties": properties,
    }
...
def _effect(prop_id: str, effect_type_ref: str, **properties: Any) -> dict[str, Any]:
    return _property(
        ...
        "RULESET_EFFECT",
        {
            "effect_type_ref": effect_type_ref,
            "properties": properties,
        },
    )
...
STARTING_ENTITIES: list[dict[str, Any]] = [
    _entity(
        ...
        [
            _data(
                ...
                "object",
                role="input",
                description="Incoming order request.",
            ),
            _function(
                "CREATE_ORDER",
                "command",
            ...
            _event(
                "EVENT_NEW_ORDER",
                "new_order",
                description="A new order has been accepted for processing.",
            ),
            _effect(
                "EFFECT_INSERT_ORDER",
                "insert",
                description="Insert the order into the Orders dataset.",
            ),
            _effect(
                "EFFECT_RESERVE_INVENTORY",
                "decrement",
                description="Reserve inventory for the accepted order.",
            ),
            _effect(
                "EFFECT_INCREMENT_CUSTOMER_ORDER_COUNT",
                "increment",
                description="Increment the customer's order count.",
            ),
            _link(
                "LINK_DEP_CUSTOMER_FOR_CREATE_ORDER",
                "RULESET_LINK_DEPENDENCY",
                "dependency",
                "CUSTOMERS_CUSTOMER_ID",
                "CREATE_ORDER",
                reason="create_order requires an addressable customer identity",
            ),
            _link(
                "LINK_DEP_INVENTORY_FOR_CREATE_ORDER",
                "RULESET_LINK_DEPENDENCY",
                "dependency",
                "INVENTORY_AVAILABLE_QUANTITY",
                "CREATE_ORDER",
                reason="create_order requires addressable inventory availability",
            ),
            _link(
                "LINK_INPUT_ORDER_REQUEST_TO_NEW_ORDER",
                "RULESET_LINK_EVENT_INPUT",
                "event_input",
                "ORDER_REQUEST",
                "EVENT_NEW_ORDER",
            ),
            _link(
                "LINK_NEW_ORDER_TO_INSERT_ORDER",
                "RULESET_LINK_EVENT_EFFECT",
                "event_effect",
                "EVENT_NEW_ORDER",
                "EFFECT_INSERT_ORDER",
            ),
            _link(
                "LINK_NEW_ORDER_TO_RESERVE_INVENTORY",
                "RULESET_LINK_EVENT_EFFECT",
                "event_effect",
                "EVENT_NEW_ORDER",
                "EFFECT_RESERVE_INVENTORY",
            ...
            _link(
                "LINK_NEW_ORDER_TO_INCREMENT_CUSTOMER",
                "RULESET_LINK_EVENT_EFFECT",
                "event_effect",
                "EVENT_NEW_ORDER",
                "EFFECT_INCREMENT_CUSTOMER_ORDER_COUNT",
            ...
            _link(
                "LINK_INSERT_ORDER_TO_RECORDSET",
                "RULESET_LINK_EFFECT_TARGET",
                "effect_target",
                "EFFECT_INSERT_ORDER",
                "ORDERS_RECORDSET",
            ),
            _link(
                "LINK_INSERT_ORDER_TO_ORDER_ID",
                "RULESET_LINK_EFFECT_TARGET",
                "effect_target",
                "EFFECT_INSERT_ORDER",
                "ORDERS_ORDER_ID",
            ...
            _link(
                "LINK_INSERT_ORDER_TO_CUSTOMER_ID",
                "RULESET_LINK_EFFECT_TARGET",
                "effect_target",
                "EFFECT_INSERT_ORDER",
                "ORDERS_CUSTOMER_ID",
            ...
            _link(
                "LINK_RESERVE_INVENTORY_TO_AVAILABLE",
                "RULESET_LINK_EFFECT_TARGET",
                "effect_target",
                "EFFECT_RESERVE_INVENTORY",
                "INVENTORY_AVAILABLE_QUANTITY",
                operation="decrement_by_order_quantity",
            ),
            _link(
                "LINK_INCREMENT_CUSTOMER_TO_ORDER_COUNT",
                "RULESET_LINK_EFFECT_TARGET",
                "effect_target",
                "EFFECT_INCREMENT_CUSTOMER_ORDER_COUNT",
    ...
    _entity(
        ...
        "datastore",
        [0.0, 6.0, 0.0],
        "Physical datastore boundary for order data.",
        "Order DB stores the Sales schema.",
    ),
    _entity(
        "SALES_SCHEMA",
        "Sales Schema",
        ...
        [
            _link(
                "LINK_ORDER_DB_CONTAINS_SALES_SCHEMA",
                "RULESET_LINK_CONTAINMENT",
                "containment",
                "ORDER_DB",
                "SALES_SCHEMA",
            )
    ...
    _entity(
        ...
        [
            ...
            _event(
                "EVENT_ORDER_PERSISTED",
                "order_persisted",
                description="The new order has been persisted.",
            ),
            _effect(
                "EFFECT_CONFIRM_ORDER",
                "set",
                description="Set the persisted order status to confirmed.",
            ),
            _link(
                "LINK_SALES_SCHEMA_CONTAINS_ORDERS",
                "RULESET_LINK_CONTAINMENT",
                "containment",
                "SALES_SCHEMA",
                "ORDERS",
            ...
            _link(
                "LINK_RECORDSET_TO_ORDER_PERSISTED",
                "RULESET_LINK_EVENT_CONDITION",
                "event_condition",
                "ORDERS_RECORDSET",
                "EVENT_ORDER_PERSISTED",
                condition="record_inserted",
            ),
            _link(
                "LINK_ORDER_PERSISTED_TO_CONFIRM",
                "RULESET_LINK_EVENT_EFFECT",
                "event_effect",
                "EVENT_ORDER_PERSISTED",
                "EFFECT_CONFIRM_ORDER",
    ...
    _entity(
        ...
        "datastore",
        [6.0, 3.0, 0.0],
        "Physical datastore boundary for inventory data.",
        "Inventory DB stores the Inventory dataset.",
    ),
    _entity(
        "INVENTORY",
        "Inventory",
        ...
        [
            ...
            _event(
                "EVENT_LOW_STOCK",
                "low_stock",
                description="Available quantity is below the replenishment threshold.",
            ),
            _effect(
                "EFFECT_CREATE_REPLENISHMENT_REQUEST",
                "enqueue",
                description="Create a replenishment request.",
            ),
            _link(
                "LINK_INVENTORY_DB_CONTAINS_INVENTORY",
                "RULESET_LINK_CONTAINMENT",
                "containment",
                "INVENTORY_DB",
                "INVENTORY",
    ...
    _entity(
        ...
        "datastore",
        [6.0, -3.0, 0.0],
        "Physical datastore boundary for customer data.",
        "Customer DB stores the Customers dataset.",
    ),
    _entity(
        "CUSTOMERS",
        "Customers",
        ...
        [
            ...
            _event(
                "EVENT_CUSTOMER_SEGMENT_RECALCULATION",
                "customer_segment_recalculation",
                description="Customer segment must be recalculated.",
            ),
            _effect(
                "EFFECT_UPDATE_CUSTOMER_SEGMENT",
                "set",
                description="Update the customer's derived segment.",
            ),
            _link(
                "LINK_CUSTOMER_DB_CONTAINS_CUSTOMERS",
                "RULESET_LINK_CONTAINMENT",
                "containment",
                "CUSTOMER_DB",
                "CUSTOMERS",
            ...
            _link(
                "LINK_ORDER_COUNT_TO_SEGMENT_RECALC",
                "RULESET_LINK_EVENT_CONDITION",
                "event_condition",
                "CUSTOMERS_ORDER_COUNT",
                "EVENT_CUSTOMER_SEGMENT_RECALCULATION",
            ...
            _link(
                "LINK_CUSTOMER_FK_RELATION",
                "RULESET_LINK_RELATION",
                "relation",
                "CUSTOMERS_CUSTOMER_ID",
                "ORDERS_CUSTOMER_ID",
                relation_type_ref="foreign_key",
            ),
        ],
    ),
]

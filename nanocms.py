...
def _canonical_projection(placement_id: str, title: str, projection_id: str, renderer: str) -> dict[str, Any]:
    return {
        "id": placement_id,
        "title": title,
...
PLACEMENTS: dict[str, dict[str, Any]] = {
    ...
    "view.spatial_dependency_3d": _canonical_projection("view.spatial_dependency_3d", "Spatial Dependency", "spatial_dependency_3d", "canonical_projection_3d"),
    ...
    "view.raw_json_tree": {"id":"view.raw_json_tree","title":"JSON Tree","type":"structureprojector_view","view":"raw_json_tree","ruleset":"RawJSON","view_ruleset":None,"renderer":"svg","context_model":"json_pointer","render_ruleset":None},
    "view.raw_json_master_map": {"id":"view.raw_json_master_map","title":"Map 2D","type":"structureprojector_view","view":"raw_json_master_map","ruleset":"RawJSON","view_ruleset":None,"renderer":"svg_master_map","context_model":"json_pointer","render_ruleset":"render.aigmos_master_map"},
}
...
def _validate() -> None:
    if ROOT_PAGE not in PAGES: raise ValueError("nanoCMS root page is missing")
    if DEFAULT_PAGE not in PAGES: raise ValueError("nanoCMS default page is missing")
    for page_id, page in PAGES.items():
        if page.get("id") != page_id: raise ValueError(f"nanoCMS page identity mismatch: {page_id}")
        for field in ("title", "menuitem", "description", "children", "placements"):
            if field not in page: raise ValueError(f"nanoCMS page {page_id} missing {field}")
        for child_ref in page["children"]:
            if child_ref not in PAGES: raise ValueError(f"nanoCMS page {page_id} has unresolved child {child_ref}")
        for placement_ref in page["placements"]:
            if placement_ref not in PLACEMENTS: raise ValueError(f"nanoCMS page {page_id} has unresolved placement {placement_ref}")
    for placement_id, placement in PLACEMENTS.items():
        if placement.get("id") != placement_id: raise ValueError(f"nanoCMS placement identity mismatch: {placement_id}")
        for field in ("title", "view", "ruleset", "renderer", "context_model", "render_ruleset", "view_ruleset"):
            if field not in placement: raise ValueError(f"nanoCMS placement {placement_id} missing {field}")
...
def resolve_page(page_id: str | None) -> dict[str, Any]:
    ...
    if resolved_id not in PAGES: raise KeyError(resolved_id)
...
def resolve_view(page_id: str | None, view_id: str | None = None) -> dict[str, Any]:
    ...
    if not placements: raise KeyError(view_id or "")
    if view_id is None: return deepcopy(placements[0])
    for placement in placements:
        if placement["id"] == view_id or placement["view"] == view_id: return deepcopy(placement)
    raise KeyError(view_id)


def navigation() -> list[dict[str, str]]:
    root = PAGES[ROOT_PAGE]
    return [{"id": child_id, "title": PAGES[child_id]["title"], "menuitem": PAGES[child_id]["menuitem"], "description": PAGES[child_id]["description"]} for child_id in root["children"]]
...
def projection(page_id: str | None = None, view_id: str | None = None) -> dict[str, Any]:
    ...
    return {"name":"StructureProjector nanoCMS","model":"recursive Page context + ordered alternative view placements","context_rule":"Changing view placement preserves the selected tree/semantic context.","root":deepcopy(PAGES[ROOT_PAGE]),"default_page":DEFAULT_PAGE,"selected_page":page,"selected_view":selected_view,"navigation":navigation()}
